...
namespace Predictor.Business
{
    public class EmailBiz
    {
        ...
        public static void SendGrid(string subject, string body, string sendTo) {
            //SendGridMessage mailMessage = new SendGridMessage();

            //mailMessage.AddTo(email);
            //mailMessage.From = new MailAddress(ConfigurationManager.AppSettings["AdminEmail"], "7 Peaks");
            //mailMessage.Subject = "You've been invited to collaborate on 7Peaks PNS";
            //mailMessage.Html = await GetConfirmationEmailBodyAsync(id, email);
